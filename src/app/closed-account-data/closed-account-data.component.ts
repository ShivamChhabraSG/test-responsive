...
@Component({
  selector: 'app-closed-account-data',
  templateUrl: './closed-account-data.component.html',
  styleUrls: ['./closed-account-data.component.css']
})
export class ClosedAccountDataComponent {
  now: any;
  bankCode: any = [];
  FYstart: any;
  FYend: any;
  ...
  disabled = false;
  selectedItems: any = [];
  constructor(private dateFormatService:DateFormatWithoutTimeService,private entityGridService: SharedEntityServiceService,private spinner:NgxSpinnerService, private fb: FormBuilder,public dialog: MatDialog,private router: Router,public datepipe: DatePipe,private entityService: EntityScreenServiceService,private datePipe: DatePipe){
...
// this.bankCode = 0
   ...
   // console.log('Entitlements', this.entitlements);
    ...
    // this.updateEndYears();
    setTimeout(() => {
      this.getClosedAccountCodesbyYear();
    }, 2000);
  ...
  // start year dropdown
  getstartdate(){
    this.entityService.getfinStartYear().subscribe(
      (res:any)=>{
    //  console.log(res);
  ...
  // getClosedAccountCodesbyYear()
  // getsghcodes by year
  getClosedAccountCodesbyYear(){
    this.spinner.show();
    if(this.bankCode === '99') {
      console.log(this.fyStart,this.fyEnd);
      this.entityService.getAllAccountsCodeByYear(this.fyStart,this.fyEnd).subscribe(
        (res:any)=>{
        this.spinner.hide();
      ...
      this.entityService.getAllClosedAccountCodesByBankYear(this.bankCode,this.fyStart,this.fyEnd).subscribe((res:any)=>{
        this.spinner.hide();
        this.sghCodes = res.data;
        console.log(res.data);
      },
  ...
  downloadRecord(){
    ...
    let bankCode = this.celldata.bank.split('-')[0].trim();
    console.log(this.celldata.bankCode,fyEnd,qtrEnd,isView)
    this.spinner.show();
    this.entityService.downloadClosedAccountsData(bankCode,fyEnd,qtrEnd,isView).then((res:any)=>{
      const blob = new Blob([res], { type: 'text/csv' });
      const url = window.URL.createObjectURL(blob);
      const link = document.createElement('a');
      link.href = url;
      link.download = this.celldata.bankCode +'-' + 'Closed_Account_Data';
      link.click();
...
// alert(fyEnd)
// alert(qtrEnd)accountsView
sessionStorage.setItem("accountsView",'ClosedAccountView');
  ...
  onCellClicked(event: CellClickedEvent) {
    ...
    let bankCode = this.celldata.bank.split('-')[0].trim();
    sessionStorage.setItem('uploadClosedDatabankCode',bankCode);
