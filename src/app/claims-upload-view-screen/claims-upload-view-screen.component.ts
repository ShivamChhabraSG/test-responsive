...
@Component({
  selector: 'app-claims-upload-view-screen',
  templateUrl: './claims-upload-view-screen.component.html',
  styleUrls: ['./claims-upload-view-screen.component.css'],
  providers: [DatePipe]
})
export class ClaimsUploadViewScreenComponent {
  now: any;
  recordStatus: any;
  FYstart: any;
  FYend: any;
  ...
  columnDefss: ColDef[] = [
    ...
    },
    {
      ...
      field: 'M1OpeningBalance',
      tooltipField: 'M1OpeningBalance',
      editable: false,
      suppressSizeToFit: true,
      resizable: true,
      width:200
...

    {
      ...
      field: 'M3OverdueAmount',
      tooltipField: 'M3OverdueAmount',
      editable: false,
      suppressSizeToFit: true,
      resizable: true,
      width:170
  ...
  ngOnInit() {
    ...
    this.regionalOffice = sessionStorage.getItem('regionalOffice')
    this.accountsView = sessionStorage.getItem("accountsView");

    if (this.accountsView === 'dataView') {
      this.getDataView();
    }
    if (this.accountsView === 'uploadView') {
     this.uploadViewId = sessionStorage.getItem("AccountsView");
  ...
  getDataView() {
    let fyEnd = sessionStorage.getItem('claimsfyEnding');
    this.fyEnd =this.datePipe.transform(fyEnd,'dd-MM-yyyy')
    let qtrEnd :any= sessionStorage.getItem('claimsqtrEnding');

    this.qtrEnd= this.datePipe.transform(qtrEnd,'dd-MM-yyyy')


    let accountDataStatusId = sessionStorage.getItem('accountDataStatusId');


    this.bankcode =sessionStorage.getItem('bankCodeUser');
    // let accountDataStatusId = sessionStorage.getItem('accountDataStatusId');


    let isView = JSON.stringify(true)
this.spinner.show();
    this.entityService.viewaccountscreenclaimsdata(this.bankcode, this.fyEnd, this.qtrEnd, isView).subscribe(
      (response: any) => {
      console.log("SelectedLevelsResponseclaims", response)
      ...
      fetch(url)
        ...
        .then(csvData => {
          ...
          console.log("CSVUserDATA", this.csvViewData.length);

          this.arrayLoop = this.csvViewData;
          console.log("arrayLoop123", this.arrayLoop);


          for (let i = 0; i < this.csvViewData.length; i++) {
            ...
            for (let i = 0; i < this.arrayLoop[0].Bank.length; i++) {
...
this.spinner.hide();
  ...
  getClosedDataView() {
    let fyEnd = sessionStorage.getItem('accountFyEnd');
    this.fyEnd = fyEnd

    let qtrEnd = sessionStorage.getItem('accountqtrEnd');
    this.qtrEnd = qtrEnd

    let bankcode = sessionStorage.getItem('uploadClosedDatabankCode');
    this.bankcode = bankcode
    let isView = JSON.stringify(true)
    this.entityService.viewaccountscreen(bankcode, fyEnd, qtrEnd, isView).subscribe(
      (response: any) => {
      ...
      fetch(url)
        ...
        .then(csvData => {
          ...
          this.csvViewData = parsedData;
          sessionStorage.setItem("viewUserCsvDAta", JSON.stringify(this.csvViewData));
          console.log('this.csvViewData',this.csvViewData.length)
          console.log("CSVUserDATA", parsedData);
          this.arrayLoop=[]
          this.arrayLoop =this.csvViewData;
          console.log('length',this.arrayLoop);
          ...
          for (let i = 0; i < this.csvViewData.length; i++) {
            ...
            for (let i = 0; i < this.arrayLoop[0]?.Branch != undefined ? this.arrayLoop[0]?.Branch.length:0; i++) {
              const char = this.arrayLoop[0].Branch[i];
              if (char >= "0" && char <= "9") {
                extractedNumber += char;
              } else if (extractedNumber !== "") {
                // Break the loop once we have extracted the number
  ...
  getUploadDataView() {
    this.entityService.viewaccountsupload(this.uploadViewId).subscribe(
      (response: any) => {
      console.log("SelectedLevelsResponse", response)
      console.log(response);
      ...
      fetch(url)
        ...
        .then(csvData => {
          ...
          // Process the CSV data
          console.log("CSV View Data", csvData);
        this.arrayLoop = parsedData;
          sessionStorage.setItem("viewUserCsvDAta", JSON.stringify(this.csvViewData));
          console.log("CSVUserDATA", this.arrayLoop.length);
  ...
  tabChanged(event: any) {
    ...
    this.entityService.getGridDataFromLevels(selectedLevel).subscribe(
      (res: any) => {
