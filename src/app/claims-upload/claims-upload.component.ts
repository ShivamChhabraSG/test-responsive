...
// this.bankCode = 0
  ...
  getClaimsUploadbyYear(){
    ...
    if(this.bankCode === '99') {
      ...
      this.entityService.getAllClaimsUploadByYear(this.fyStart,this.fyEnd).subscribe(
        (res:any)=>{
        this.spinner.hide();
      ...
      this.entityService.getClaimsUploadByYear(this.bankCode,this.fyStart,this.fyEnd).subscribe(
        (res:any)=>{
        this.spinner.hide();
        this.sghCodes = res.data;
        console.log(res.data);
  ...
  //  console.log(this.celldata.bankCode,fyEnd,qtrEnd,isView)
    this.spinner.show();
    this.entityService.downloadClaimsUploadData(this.celldata.bankCode,fyEnd,qtrEnd,isView).then((res:any)=>{
      const blob = new Blob([res], { type: 'text/csv' });
      const url = window.URL.createObjectURL(blob);
      const link = document.createElement('a');
      link.href = url;
      link.download = this.celldata.bankCode +'-' + 'Claims_Upload_Data';
      link.click();
...
// alert(qtrEnd)
  ...
  UploadEntity() {
    ...
    sessionStorage.setItem('uploadFileType','claimsUpload');
  ...
  onCellClicked(event: CellClickedEvent) {
   ...
   // console.log('UnitCOde', unitCode);
    ...
    sessionStorage.setItem('claimsfyEnding', event.data.fyEnding);
    sessionStorage.setItem('claimsqtrEnding', event.data.qtrEnding);
